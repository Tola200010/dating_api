...
namespace API.Entities
{
    public class MessageRepository : IMessageRepository
    {
        ...
        {
            return await _applicationDbContext.Groups!.Include(c=>c.Connections)
        ...
        {
            return await _applicationDbContext.Messages!.Include(u => u.Sender).Include(u => u.Recipient).SingleOrDefaultAsync(x => x.Id == id);
        }

        public async Task<PagedList<MessageDto>> GetMessageForUser(MessageParams messageParams)
        {
            var query = _applicationDbContext.Messages!.OrderByDescending(m => m.MessageSent)
        ...
        {
            return await _applicationDbContext.Groups!.Include(x=>x.Connections).FirstOrDefaultAsync(x=>x.Name == groupName);
        }

        public async Task<IEnumerable<MessageDto>> GetMessageThread(string currentUsername, string recipientUsername)
        {
            var messages = await _applicationDbContext.Messages!
            .Where(m => m.Recipient!.Name == currentUsername && !m.RecipientDeleted && m.Sender!.Name == recipientUsername
            || m.Recipient!.Name == recipientUsername && m.Sender!.Name == currentUsername && !m.SenderDeleted
        ...
        {
            _applicationDbContext.Connections!.Remove(connection);
        }
    }
}